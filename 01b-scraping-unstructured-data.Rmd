...
amounts <- html_nodes(bribes, ".paid-amount span") # identify the CSS selector. The last argument is what we got from the "Selector gadget" Chrome extension.
...
```{r}
transaction <- html_nodes(bribes, ".transaction a") #The last argument is what we got from the "Selector gadget" Chrome extension.
...
for (i in 2:length(pages)){
	...
	# wait a couple of seconds between URL calls not to flood the server.
	Sys.sleep(2)
}
```
